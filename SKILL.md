...
- python3 "$ALEX_EXTENSIONS_DIR/image-gen/scripts/chroma_key_transparent.py" --input "<원본>" --out "$TMP/out.png" --key magenta --white-check "$TMP/check-white.png"
- stdout 에 `mode=RGBA` 와 `stale_transparent_rgb_pixels=0` 가 있는지 확인
...
mv "$TMP/out.png" "$DEST"
cp "$TMP/check-white.png" /tmp/_check_white.png
...
기본은 위 single-shot codex exec 패턴. 다음 두 시나리오에서는 별도 모드 — 이 skill folder 의 `scripts/` 아래 spike 스크립트들을 참조 구현으로 사용한다.
